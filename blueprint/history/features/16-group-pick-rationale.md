...
- `npm test`: 306/306 passing (14 new - 6 for `rationale.ts`'s prompt
  builder and degrade-to-`null` paths, 8 for the route's status-code
  branches).
- `npm run build` and `npm run lint`: clean (one pre-existing, unrelated
  warning in `SiteHeader.tsx`).
- Dev-server smoke test: `POST /api/sessions/[id]/rationale` 401s with no
  auth cookie, and `/sessions/[id]` still 307-redirects for an anonymous
  visit - same pattern 14c/15 used to confirm the new route/wiring didn't
  break existing auth handling.

One implementation deviation from the spec: Step 3 originally planned a
`useEffect` to reset the rationale panel when the top pick changes under the
slider. ESLint's `react-hooks/set-state-in-effect` flagged that as a
cascading-render anti-pattern, so `GroupPickRationale` is instead keyed on
`rankedMovies[0].id` in `RecommendationsPanel`, which remounts it fresh -
same effect, no effect hook.

Checkpoint commits `d74f402`, `30614fc`, `d88e514` on
`feature/group-pick-rationale`.

Known follow-up, unchanged from prior features: no Playwright in this
project, so clicking "Why this pick?" and reading the returned paragraph in
a real browser hasn't been verified in this environment - worth a manual
`/try` pass.
